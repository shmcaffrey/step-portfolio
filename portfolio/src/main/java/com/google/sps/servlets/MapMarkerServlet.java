...
public class MapMarkerServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    ArrayList<ArrayList<Double>> pairList = new ArrayList<ArrayList<Double>>();
    

    for (Entity entity : results.asIterable()) {  
        ArrayList<Double> latLngPair = new ArrayList<>();
        Double latEntry = (Double) entity.getProperty("lat");
        Double lngEntry = (Double) entity.getProperty("lng");
        latLngPair.add(latEntry);
        latLngPair.add(lngEntry);
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    ArrayList<Double> pairLatLng = getCoords(request);
    Long timestamp = System.currentTimeMillis();
    Entity markerEntity = new Entity("map-marker");
    markerEntity.setProperty("lat", pairLatLng.get(0));
    markerEntity.setProperty("lng", pairLatLng.get(1));
  ...
  private String convertJsonUsingGson(ArrayList<ArrayList<Double>> latLngPair) {
    String json = new Gson().toJson(latLngPair);
    return json;
  }

  private ArrayList<Double> getCoords(HttpServletRequest request) {
      ...
      try {
          pair.add(Double.parseDouble(latInputStr));
          pair.add(Double.parseDouble(lngInputStr));
      } catch (NumberFormatException e) {
          System.err.println("Could not convert to Double: " + latInputStr + "or " + lngInputStr);
      }

      log("pair requested: " + pair.get(0) + " " + pair.get(1));
      return pair;
  }
}
